...
fail() { printf 'section-00: ERROR: %s\n' "$*" >&2; exit 1; }
usage() { printf '%s\n' 'usage: scripts/section-00.sh <doctor|generate|build> [--of-root PATH] [--project starter|solution] [--configuration Debug|Release]'; }
[[ $# -gt 0 ]] || { usage; exit 2; }
COMMAND=$1; shift
while [[ $# -gt 0 ]]; do
  case "$1" in
...
PROJECT_PATH="$ROOT/exercises/00-visual-signature/$PROJECT"

validate_source_inventory() {
  local expected actual source_root="$ROOT/exercises/00-visual-signature"
...
resolve() {
  ...
  case "$(uname -s):$(uname -m)" in
    ...
    *) fail "unsupported host $(uname -s) $(uname -m); use section-00.ps1 on Windows" ;;
  esac
...
doctor() {
  ...
  [[ -f "$constants" ]] || fail 'OF_ROOT is not an openFrameworks tree'
  ...
  [[ ! -s "$PROJECT_PATH/addons.make" ]] || fail 'section 00 projects must not enable addons'
  [[ -f "$ROOT/exercises/00-visual-signature/shared/signature_geometry.cpp" ]] || fail 'shared geometry source is missing'
  validate_source_inventory
  printf 'section-00: host=%s project=%s OF=%s PG=0.103.0\n' "$PLATFORM" "$PROJECT" "$version"
}

clean_generated() {
  [[ "$PROJECT_PATH" == "$ROOT/exercises/00-visual-signature/"* && ! -L "$PROJECT_PATH" && ! -L "$PROJECT_PATH/bin" ]] || fail 'unsafe project path'
...
../shared/signature_geometry.h
src/design/signature_design.cpp
src/design/signature_design.h
src/main.cpp
src/ofApp.cpp
src/ofApp.h
EOF
  ...
  for source in src/main.cpp src/ofApp.cpp src/design/signature_design.cpp ../shared/signature_geometry.cpp; do
...
validate_generated() {
  ...
  case "$PLATFORM" in
    linux64)
      ...
      [[ "$external_root" == "$ROOT/exercises/00-visual-signature/shared" ]] || fail 'generated external source root escaped the exercise shared directory'
      ;;
    osx)
...
canonical_snapshot() {
  find "$PROJECT_PATH/src" "$PROJECT_PATH/bin/data" "$ROOT/exercises/00-visual-signature/shared" -type f -print | LC_ALL=C sort | while IFS= read -r file; do cksum "$file"; done
  cksum "$PROJECT_PATH/addons.make"
}

generate() {
  doctor
  local before after before_inputs after_inputs log="$ROOT/.harness/logs/generate-$PLATFORM-section-00-$PROJECT.log"
  ...
  set +e
  (cd "$ROOT" && PG_OF_PATH="$OF_ROOT" "$PG" -o"$OF_ROOT" -p"$PLATFORM" -s"$sources" "exercises/00-visual-signature/$PROJECT") >"$log" 2>&1
  status=$?; set -e
  ...
  printf 'section-00: generated %s for %s\n' "$PROJECT" "$PLATFORM"
}
