...
void ofApp::draw() {
    ...
    ofSetLineWidth(state_.reduced_motion ? 2.0f : 4.0f);
    ...
    const float trail_x = state_.position.x - state_.velocity.x * trail_seconds;
    const float trail_y = state_.position.y - state_.velocity.y * trail_seconds;
    ofDrawLine(state_.position.x, state_.position.y - 4.0f, trail_x, trail_y - 4.0f);
    ofDrawLine(state_.position.x, state_.position.y + 4.0f, trail_x, trail_y + 4.0f);
    ofSetColor(design_.mark_color.red, design_.mark_color.green, design_.mark_color.blue);
    const float x = state_.position.x;
    const float y = state_.position.y;
    const float radius = design_.radius_pixels;
    ofDrawTriangle(x, y - radius, x + radius, y, x, y + radius);
    ofDrawTriangle(x, y - radius, x - radius, y, x, y + radius);
    if (state_.paused) {
        ofNoFill();
        ofSetLineWidth(3.0f);
