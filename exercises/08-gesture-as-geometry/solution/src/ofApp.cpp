...
void ofApp::setup() {
    ofSetWindowTitle("Section 08 solution: curvature facets");
    ofSetFrameRate(60);
...
void ofApp::draw() {
    ...
    for (std::size_t i = 1; i < gesture_.samples.size(); ++i) {
        ...
        if (!gesture::facetTriangle(a, b, style.width, triangle)) continue;
        ofSetColor(asColor(style.color));
        ofDrawTriangle(triangle.first.x, triangle.first.y,
                       triangle.second.x, triangle.second.y,
                       triangle.tip.x, triangle.tip.y);
        ofSetColor(asColor(design_.slow_color));
        ofDrawCircle(b.x, b.y, std::max(1.0f, style.width * 0.22f));
    }
    ofNoFill(); ofSetColor(asColor(design_.fast_color)); ofSetLineWidth(1.0f);
    ofDrawRectangle(keyboard_cursor_.x - 4.0f, keyboard_cursor_.y - 4.0f, 8.0f, 8.0f);
}
