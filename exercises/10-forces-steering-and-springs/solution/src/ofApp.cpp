...
ofColor asColor(forces::Color color) { return ofColor(color.r, color.g, color.b); }
}

void ofApp::setup() {
    ofSetWindowTitle("Section 10 solution: elastic constellation");
    ofSetFrameRate(60);
...
void ofApp::resetScene() {
    forces::reset(system_, {ofGetWidth() * 0.5f, ofGetHeight() * 0.5f},
                  forces::BehaviorMode::spring_chain, design_,
...
void ofApp::moveControl(float x, float y) {
    ...
    if (!forces::clampPointToBounds({x, y}, bounds, control_inset, point)) return;
    system_.target = point;
    system_.anchor = point;
}

void ofApp::update() {
    forces::advanceFrame(system_, ofGetLastFrameTime(), design_,
                         {static_cast<float>(ofGetWidth()), static_cast<float>(ofGetHeight())});
}
...
void ofApp::keyPressed(int key) {
    if (key == '1') forces::setMode(system_, forces::BehaviorMode::seek);
    else if (key == '2') forces::setMode(system_, forces::BehaviorMode::spring_chain);
    else if (key == ' ') forces::setMode(system_, system_.mode == forces::BehaviorMode::seek ? forces::BehaviorMode::spring_chain : forces::BehaviorMode::seek);
    else if ((key == 'p' || key == 'P') && !reduced_motion_) forces::setPaused(system_, !system_.paused);
    else if (key == 'r' || key == 'R') resetScene();
    else if (key == 'm' || key == 'M') { reduced_motion_ = !reduced_motion_; forces::setPaused(system_, reduced_motion_); }
    else {
...
void ofApp::draw() {
    ...
    ofSetColor(asColor(spring ? design_.spring_color : design_.seek_color));
    if (!reduced_motion_ && !system_.agents.empty()) {
        ofNoFill();
        ofSetLineWidth(2.0f);
        if (spring) {
            ...
            for (std::size_t index = 1; index < system_.agents.size(); ++index) {
                ofDrawLine(system_.agents[index - 1].position.x,
                           system_.agents[index - 1].position.y,
                           system_.agents[index].position.x,
                           system_.agents[index].position.y);
            }
        } else {
    ...
    for (std::size_t index = 0; index < system_.agents.size(); ++index) {
        const auto& point = system_.agents[index].position;
        if (spring) {
            ofNoFill();
            ofSetLineWidth(3.0f);
            ofDrawRectangle(point.x - mark_radius,
                            point.y - mark_radius,
                            mark_radius * 2.0f,
                            mark_radius * 2.0f);
        } else {
            ...
            ofRotateDeg(static_cast<float>(index) * 29.0f);
            ofFill();
            ofDrawTriangle(0, -mark_radius,
    ...
    ofDrawBitmapString("pointer/arrows target or anchor | 1 seek triangles/ring | 2 spring squares/square | space switch | P/R/M", 12, 22);
}
