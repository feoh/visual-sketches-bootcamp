...
try {
    ...
    $Starter = Join-Path $Root "exercises/00-visual-signature/starter/src/design"
    $TestSource = Join-Path $Root "exercises/00-visual-signature/tests/signature_geometry_test.cpp"
    Push-Location $Build
    try {
        & $Compiler.Source /nologo /std:c++14 /Zc:__cplusplus /EHsc "/I$Shared" "/I$Starter" `
        ...
        & $Compiler.Source /nologo /std:c++17 /Zc:__cplusplus /EHsc /W4 /WX "/I$Shared" "/I$Starter" `
            (Join-Path $Shared "signature_geometry.cpp") `
            (Join-Path $Starter "signature_design.cpp") `
            $TestSource `
