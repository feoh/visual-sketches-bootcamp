...
$FakeOf = Join-Path ([IO.Path]::GetTempPath()) ("section-01-incomplete-of-" + [guid]::NewGuid().ToString("N"))
$Sentinel = Join-Path $Root "exercises\01-a-mark-that-moves\starter\bin\wrapper-safety-sentinel"
try {
    ...
    $Rejected = $false
    try {
        & (Join-Path $Root "scripts\section-01.ps1") generate -OfRoot $FakeOf -Project starter
    } catch {
        $Rejected = $true
    }
    ...
    Write-Host "section-01-wrapper-safety: incomplete OF_ROOT rejected before cleanup; sentinel preserved"
} finally {
