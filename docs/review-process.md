...
| BASELINE-READER | 2026-08-18 | repository baseline | pedagogy | complete curriculum | reader-facing | high | no independent reader review recorded | not observed | Automated checks do not establish how approachable the prose feels. | unassigned | Keep the writing beginner-friendly and review it directly while making sketches. |
...
| DELIVERY-AUTO-002 | 2026-08-19 | working tree based on `5afa25b` | intake | sections 00–17, route contract, and optional site | automated | high | required Hugo check; 21 negative authoring contracts; publication build; every section 00–15 and first-C++ starter/solution under GCC 16 and Clang 22; ShellCheck | local-pass-awaiting-commit | All commands passed locally; Hugo emitted 19 lessons and the checked sibling synthesis link. PowerShell and native OF builds were not run locally. | delivery implementation review | Commit the revision, obtain Windows contract and three-platform native CI, then record immutable run URLs. |
...
| DELIVERY-CACHYOS-MANUAL-001 | 2026-08-18 | working tree based on `64da096` | platform/tooling | foundation and section 00 | manual graphical/device | high | two five-second runtime probes with rendering and resize observation | local-pass-awaiting-commit | The author confirmed foundation and section-00 rendering and resize after the patch. This was a technical smoke check, not a reader walkthrough. | delivery implementation review | Keep technical smoke checks separate from prose and accessibility review. |
...
| DELIVERY-A11Y-HOME-001 | 2026-08-19 | `f8b9fe8` deployed at <https://feoh.github.io/visual-sketches-bootcamp/> | accessibility | publication home page only | automated browser | high | headless Chromium 151 screenshots at 320/640/1280 CSS pixels, light and dark `prefers-color-scheme`, print-to-PDF, and a CDP run that dispatches a real Tab keypress | fixed-awaiting-verification | First Tab moves focus to the skip link, which becomes visible on screen at 16,16 (187x50) with a solid 3px `#ffad66` outline. The hero image loads (`complete`, non-zero natural size), carries 151 characters of alt text, and does not overflow the viewport at any tested width; the header stacks and the figure scales at 320 pixels. Print output hides the header, skip link, and pagination and appends link URLs. Dark and light schemes both render the dark navy artwork legibly. | delivery implementation review | Automated browser evidence for the home page only. It does not cover lesson pages, does not exercise a screen reader, and is not the human keyboard/AT walkthrough the readiness gate requires. In-image labels in the SVG become unreadably small at 320 pixels; the alt text carries that meaning instead. |
| DELIVERY-CI-002 | 2026-08-19 | `9ba59bb4fd7ab6051cfbac74a32d7ba4646450aa` | intake | repository rename, both authoring checkers, sections 00–17 | automated | high | [GitHub Actions run 32215298372](https://github.com/feoh/visual-sketches-bootcamp/actions/runs/32215298372), including one failed-job rerun | verified | All 50 jobs pass: POSIX and Windows contracts plus 48 native section jobs across Linux, macOS, and Windows. `section-10-linux-build` first failed inside `Install Linux dependencies` after its second bounded attempt, before generation or compilation, and passed on rerun — the same apt/dpkg flakiness recorded in DELIVERY-CI-001, not a change defect. This is the first fully green authoring matrix on a delivery-head commit. | delivery implementation review | Treat the apt installer as a known flaky dependency, not as evidence about section 10. |
| DELIVERY-PS-PARITY-001 | 2026-08-19 | `9ba59bb4fd7ab6051cfbac74a32d7ba4646450aa` | platform/tooling | `scripts/check-authoring.ps1` | automated | high | Windows contract job in [run 32215298372](https://github.com/feoh/visual-sketches-bootcamp/actions/runs/32215298372) | verified | The Windows checker had been silently weaker than the POSIX one. `Where-Object` written without whitespace around its operator parsed as a bareword property name, so route membership matched zero rows and the repeated-lesson, instructional-heading, and duplicate source/path/url checks iterated empty sets and never tested anything; a negative `-split` limit also broke the six-field row check. Script blocks and an unlimited split restore parity. | delivery implementation review | A parity script exiting 0 is not evidence it ran its checks; prefer assertions with a positive expected count. |
